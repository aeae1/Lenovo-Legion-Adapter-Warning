# Version 1.0.0 — staged USB setup

Read `START-HERE.md` in the Version 1.0.0 download first. This release uses the unchanged V4 helper, tested on one Legion 5 Pro 16ACH6H / 82JQ with GKCN65WW. Each new machine still needs these staged checks before internal installation. Existing successful internal installations do not need to repeat them.

## Phase 1: read-only diagnostic

Use the normal Lenovo charger. Confirm Legion 5 Pro 16ACH6H / 82JQ and GKCN65WW in Windows System Information (`msinfo32`). Read encryption status with `manage-bde -status C:` in an administrator terminal and have the applicable recovery key available privately before boot-setting changes. Disable Secure Boot without clearing its keys.
...
Also check normal USB keyboard/dock operation. The patched callback performs a USB-controller connection step before displaying the warning, so bypassing the entire callback skips that step too. The owner reported a working mouse and thumb drive after internal startup. Broader controller and dock behavior remains untested; static analysis or emulation cannot establish it.
...
These USB stages do not install to the internal drive. After the diagnostic, manual and automatic comparison succeed on the new machine, follow `INTERNAL_SETUP/READ-ME-FIRST.md` from the Version 1.0.0 download. The internal setup reuses the same verified helper and includes recovery instructions. Secure Boot signing is separate; read `SIGNING-WALKTHROUGH.md`. No stage flashes BIOS, changes the EC, or replaces Windows Boot Manager.

For help interpreting an unexpected result, see [troubleshooting](TROUBLESHOOTING.md).
